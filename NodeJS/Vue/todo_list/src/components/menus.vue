<template>
  ...
  <div class="list-todos">
    ...
    <a v-for="(item,index) in todoList" :key="index" :class="{'active': item.id === todoId}"
       class="list-todo list activeListClass" @click="goList(item.id)">
      <!-- 把以前的item换成todoList -->
      <span v-if="item.locked" class="icon-lock"></span>
      <span v-if="item.count > 0" class="count-list">{{ item.count }}</span>
      {{ item.title }}
    </a>
