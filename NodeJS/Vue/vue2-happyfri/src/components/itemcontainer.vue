        ...
        <div v-if="itemDetail.length > 0" class="item_list_container">
          ...
          <ul>
            <li v-for="(item, index) in itemDetail[itemNum-1].topic_answer"
                class="item_list" @click="choose(index, item.topic_answer_id)">
              <span class="option_style" v-bind:class="{'has_choose':chooseNum==index}">{{ chooseType(index) }}</span>
              <span class="option_detail">{{ item.answer_name }}</span>
            </li>
          </ul>
        </div>
      </div>
      <span v-if="itemNum < itemDetail.length" class="next_item button_style" @click="nextItem"></span>
